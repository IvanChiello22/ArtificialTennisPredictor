...
#Calcolo probabilità basate sulle quote

# Converte i valori in float, quindi in int
df_cleaned['Odd_1'] = df_cleaned['Odd_1'].astype(str).str.replace(',', '.').astype(float).astype(int)
df_cleaned['Odd_2'] = df_cleaned['Odd_2'].astype(str).str.replace(',', '.').astype(float).astype(int)
...
# Calcola l'overround
df_cleaned['overround'] = df_cleaned['p1'] + df_cleaned['p2']

# Normalizza le probabilità
df_cleaned['prob_1'] = df_cleaned['p1'] / df_cleaned['overround']
df_cleaned['prob_2'] = df_cleaned['p2'] / df_cleaned['overround']


# Eliminazione delle righe con valori nulli ea evitare
df_cleaned = df_cleaned[~df_cleaned.isin([-1]).any(axis=1)]

df_cleaned.drop(['p1', 'p2', 'overround', 'Odd_1', 'Odd_2'], axis=1, inplace=True)
